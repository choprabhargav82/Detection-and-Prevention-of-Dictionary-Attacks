...
const char *Algorithms_Hash_MD5(const char *msg, int mlen)
{
...
int main(int argc, char *argv[])
{
	...
	for (i1 = 0; i1 < n1; i1++)
	{
		...
		strcat(salt, inp);
		strcpy(str, Algorithms_Hash_MD5(salt, strlen(salt)));
		fprintf(file_ptr, "%s", str);
		fprintf(file_ptr, "\n");
		printf("\nEncrypted hash code: 0x%s", str);
	}
	fclose(file_ptr);
	printf("\n");
	printf("\n\t MD5 Encyption Completed Successfully!!!\n\n");
	getch();
	return 0;
}
